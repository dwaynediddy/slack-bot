...
# Function to load and send FAKE_DATA messages
def load_and_send_fake_messages():
    cursor.execute("SELECT message, sender FROM my_slack_bot")
    fake_messages = cursor.fetchall()

    for message, sender in fake_messages:
...
def schedule_message():
    ...
    load_and_send_fake_messages()
    
    # can replace the line above if you want multiple scheduled times
    if (
        (current_time.weekday() == 3 and current_time.hour == 15 and current_time.minute == 40) or
        (current_time.weekday() == 3 and current_time.hour == 15 and current_time.minute == 26) or
        (current_time.weekday() == 3 and current_time.hour == 15 and current_time.minute == 42)   
    ):
        send_latest_unsent_dms()
